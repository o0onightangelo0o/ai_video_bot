...
async def cmd_stats(message: Message, svc: Services) -> None:
    ...
    await message.answer(
        "📈 <b>Bot statistics</b>\n\n"
        f"👥 Users: {s['users_total']} (active 24h: {s['users_24h']})\n"
        f"🎬 Jobs: {s['jobs_total']} (24h: {s['jobs_24h']})\n"
        f"   ✅ done: {s['jobs_done']}  ❌ failed: {s['jobs_failed']}  ⏳ active: {s['jobs_active']}\n"
        f"🚫 Banned: {s['banned']}\n"
        f"📦 Queue now: {svc.queue.size}\n"
